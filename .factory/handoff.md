...
- Corrected commit `2a833a8fda3d355b18cd8c1cfcf9bc657b8af9f6` built successfully in ACR run `ch1y6`. Only this product's locked revisions `0000008` and `0000009` were deactivated to release the legacy connection. Revision `sf-ap-ready-invoice--0000010` then became Healthy with one replica and 100% traffic; both production hostnames returned the corrected SHA. Its startup log shows the migration lock completed, the persisted `/data` key was reused, and the listener bound on port 8080.
- The evidence-only follow-up deployment after this handoff entry is the rolling-update proof: it must become healthy while `0000010` remains available, without manual revision deactivation or a `PoolTimedOut` startup failure.
